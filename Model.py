...
class InfoMaxEncoder(torch.nn.Module):
    def __init__(self, input_sz, img_ch, stt_ch=4, layer_num='deduction'):
        super().__init__()
        ...
        for i in range(0, the_layer_num - tmp):
            if i == 0:
                self.the_M.add_module(
                    ...
                    module=nn.Conv2d(img_ch, stt_ch * (2 ** i), 4, 2, 1))
                ...
                self.the_M.add_module(
                    name="MConv{:d}".format(i),
                    module=nn.Conv2d(stt_ch * (2 ** (i - 1)), stt_ch * (2 ** i), 4, 2, 1))
                self.the_M.add_module(name="MBN{:d}".format(i),
                                      module=nn.BatchNorm2d(stt_ch * (2 ** i), 1.e-3))
                self.the_M.add_module(name="MAct{:d}".format(i), module=self.relu)
    ...
    def forward(self, x):
        mapM = self.the_M(x)
        mapY = self.the_Y(mapM).squeeze(-1)
        return mapY, mapM
...
class GlobalDiscriminator(torch.nn.Module):
    ...
    def __init__(self, M_ch_num, M_shape, E_sz, inter_ch=512):
        super().__init__()

        in_channels = M_ch_num
        ...
        in_feature = out_channels * (M_shape[0] - 2 * 2) * (M_shape[1] - 2 * 2) + E_sz
        self.l0 = torch.nn.Linear(in_feature, inter_ch)
        self.l1 = torch.nn.Linear(inter_ch, inter_ch)
        self.l2 = torch.nn.Linear(inter_ch, 1)
...
class LocalDiscriminator(torch.nn.Module):
    def __init__(self, M_ch_num, E_sz, inter_ch=512):
        super().__init__()

        in_channels = M_ch_num + E_sz
        self.c0 = torch.nn.Conv2d(in_channels, inter_ch, kernel_size=1)
        self.c1 = torch.nn.Conv2d(inter_ch, inter_ch, kernel_size=1)
        self.c2 = torch.nn.Conv2d(inter_ch, 1, kernel_size=1)
...
class DeepInfoMaxLoss(torch.nn.Module):
    ...
    def get_models(self, input_shape=(32, 32), num_feature=64, out_size=64, interm_size_G=512, interm_channels_L=512,
        ...
        self.encoder = InfoMaxEncoder(input_sz=64, img_ch=1)
